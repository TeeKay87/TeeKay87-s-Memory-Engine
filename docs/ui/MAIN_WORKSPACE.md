...
- **Row 2 — host/tool actions:** **Reload Plugins** -> capability-driven **Disassembler...** -> capability-driven **Debugger...**. From `0.1.7.rev43`, Disassembler and Debugger use the theme-driven Primary button role used by the active First Scan action; Reload Plugins remains Secondary. Future permanent top-level tool/action buttons are appended to this same second row instead of creating a third permanent row.
...
## Rev33 Disassembler and Call Stack Comparer Workflow

Host `0.1.7.rev35` keeps the already modeless debugger/tool workflow without adding another permanent main-window panel. A single selected Disassembler instruction exposes separate **Add Breakpoint** and **Add Watchpoint** actions. Add Breakpoint reuses the existing attached-debugger validation/add path for a Software/Execute request. Rev38 allows Add Watchpoint to evaluate any single selected instruction while the matching debugger is Paused with current registers; the optional platform resolver must still derive a safe effective memory address, access mode, and width, after which the normal debugger validator remains authoritative. Base/index results use the current paused register values, so they are current-context candidates rather than historical execution claims. An Extended selection of two or more instructions disables both actions while leaving all existing multi-row copy/export behavior intact.

The Debugger's Call Stack workspace exposes **Compare...**. It opens or reactivates a separate modeless **Call Stack Comparer** associated with that Debugger. From rev39, opening/reopening the comparer never captures automatically; it only displays the snapshot collection already owned by that Debugger-session workspace. The collection survives comparer-window close/reopen for as long as the same Debugger window/session remains alive. **Capture Current** is the only live-capture action, is disabled while Running, and re-enables when a new valid Paused stop context arrives. Closing the Debugger detaches an already-open comparer from live authority while preserving its snapshots for offline analysis; a later/reconnected Debugger session does not inherit them as live state.

From rev42, each snapshot Group dropdown has an explicit new-group text input. Rev43 adds a **No group** action above it so membership can be removed explicitly without deleting the session group name. Existing group names remain normal choices below the creation input. Group A and Group B only select from registered group names and never expose group creation. The application also uses the supplied TK artwork as the executable and WPF window icon.

The comparer keeps its snapshot collection visible above a proportional splitter and a derived comparison-result table below. Snapshot rows use the same always-visible control pattern used by Saved Addresses where applicable: Label and Notes are in-row text editors, Group is a dropdown, the immutable evidence columns remain read-only, and every row has its own Danger-styled Remove button. From rev42, opening Group places keyboard focus in a dedicated first-row text input; entering a non-empty name and pressing Enter registers/reuses that comparer-session group and assigns it to the originating snapshot. Existing registered names are ordinary choices below the input. Group-name reuse is case-insensitive, imported Group metadata joins the session catalog, and snapshots may be reassigned without recapture/reimport. Group A and Group B are non-editable dropdowns populated only from registered session names, so users cannot create a group from the comparison selectors. Multiple snapshots assigned the same name are passed together to the existing grouped comparison engine. Pairwise comparison still uses exactly two selected snapshots. Rev43 now gates **Compare 2**, **Compare Groups**, complete-snapshot **Export...**, **Remove All**, and **Export Results...** to their valid states and invalidates stale pair/group results when the underlying comparison inputs change. Import/export uses complete snapshot JSON, while **Export Results...** uses the existing Universal Export dialog/pipeline.
