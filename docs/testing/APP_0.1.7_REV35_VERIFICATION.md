# Application 0.1.7.rev35 Verification — Disassembler Debugger Actions and Watchpoint Hit-Stop Highlighting

## Purpose

Rev35 refines the Disassembler workflow before the remaining debugger-finalization acceptance continues. It carries the complete rev32-rev34 functionality and adds safe instruction-derived Disassembler watchpoint creation plus separate visual presentation of the resolved watchpoint hit and the real stop/current IP.

Rev34 passed **152/152** automated checks. During focused runtime verification, rev32's Mock and live-PS5 trigger/current-IP separation passed, including a live `DisassemblyDerived` trigger at `0x8033D981` with real stop/current IP `0x8033D984`, followed by a successful Software/Execute breakpoint regression. Because rev35 changes Plugin API/PS5 code, start again at Gate 1.
...
1. Extract rev35 into a clean folder.
...
Rev35 may be marked verified only after all eleven gates pass. Only then should the remaining `0.1.7` debugger-finalization block be considered verified and ready for the normal version-completion decision.
