# Application 0.1.7.rev37 Verification — PS5 Watchpoint Access Verification Correction

Rev37 follows the successful rev36 Windows build/test attempt that reached 151/152. The remaining failure was a verification expectation error: `add [rax+40h],esi` is read-modify-write and must resolve to `ReadWrite`, not `Write`. Runtime resolver code is unchanged in rev37. Gate 1 must still be rerun completely before runtime verification continues.
...
1. Extract rev37 into a clean folder.
