# Application 0.1.7.rev34 Verification — Debugger Finalization Verification Test Corrections

## Purpose

Rev34 carries the complete rev32-rev33 debugger-finalization implementation and corrects two deterministic automated-test assertions discovered during the first rev33 Windows run. Runtime behavior is otherwise unchanged. Verification must therefore be performed in dependency order. Do not skip directly to snapshots/comparison: if the underlying event semantics are wrong, the persisted evidence will also be wrong.

Rev31 is the latest fully verified live-PS5 teardown baseline. Rev34 must preserve that safety behavior.

## Package identity

| Item | Expected |
| --- | --- |
| Application | `0.1.7.rev34` |
| Feature | `Debugger Finalization Verification Test Corrections` |
| Plugin API | `2.17.0` |
| Mock | `1.0.0.rev17` |
| PS5 | `0.1.0.rev39` |
...
Do not continue runtime acceptance after a failed build or automated check. The two deterministic rev33 failures are corrected here. If `PS5 debugger general register snapshot protocol` again reports 28 instead of 76 registers, stop and preserve the full output; that intermittent optional-register probe result requires separate investigation before runtime gates continue.
...
This is mandatory before rev34 acceptance.
...
Rev34 may be marked verified only after:
