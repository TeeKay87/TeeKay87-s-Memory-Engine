...
The current development candidate is **TeeKay87's Memory Engine 0.1.7.rev42 — Call Stack Group Creation and Application Icon**. Rev31 remains the latest fully verified PS5 hardware-watchpoint teardown baseline. Rev38 passed the complete Windows automated gate at **152/152**, and the carried rev32 trigger/current-IP semantics, green Hit/yellow Stop presentation, and focused live PS5 Disassembler Add Breakpoint/Add Watchpoint checks have passed. Rev39 corrected Call Stack Comparer session ownership and passed the snapshot/comparer runtime gates through JSON round-trip, offline ownership, and invalid schema/version rejection. Rev40 corrected the shared Export dialog Scope/Format presentation and passed **152/152**, with its cross-consumer UI check still pending. Rev41 retained that correction and added session-local snapshot group assignment, passing **152/152**, but its first runtime UI check showed that group creation was not discoverable. Rev42 keeps the same grouping model, adds a dedicated first-row creation input inside each snapshot Group dropdown, and installs the supplied TK artwork as the application icon. Plugin API remains `2.18.0`, Mock remains `1.0.1.rev17`, PS5 remains `0.1.2.rev39`, and the registry remains **152 checks**. Optional Deep Snapshot pointer-memory analysis remains a future extension and is not part of the 0.1.7 Definition of Done.
...
- **[TK87ME — Implemented / retained by current rev42 candidate] Plugin API `2.18.0`.** API `2.15.0` added optional `DebuggerEvent.TriggeredBreakpoint`; API `2.16.0` added optional `IDebuggerBreakpointValidationService`; API `2.17.0` added neutral `TriggerInstructionAddress` / `DebuggerTriggerResolution` event semantics and explicit watched-address/access/size context; API `2.18.0` adds optional `IDisassemblyWatchpointResolver` plus neutral `DisassemblyWatchpointTarget`, allowing platform plugins to derive a safe data-watchpoint address/size/access tuple from one disassembled instruction and a current register snapshot without moving architecture-specific operand rules into Core/WPF.
- **[TK87ME — Implemented / verified baseline; rev30 classification retained by current rev42 candidate] Combined Breakpoints / Watchpoints workspace.** Add selects Software Execute Breakpoint or Hardware Watchpoint according to advertised capabilities. Hardware requests expose neutral Access and Size inputs; plugins remain authoritative for supported access modes, widths, alignment, mapped-range rules, and slot limits. The table classifies each row by semantic Type (`Breakpoint`/`Watchpoint`) and separately exposes implementation Mechanism (`Software`/`Hardware`). Refresh/Enable/Disable/Remove/Remove All work across both kinds. Disassembler navigation remains Execute-only.
- **[TK87ME — Implemented in rev30 / retained by current rev42 candidate] Debugger address actions.** Scan Results and Saved Addresses expose Add Breakpoint/Add Watchpoint only when an already-open Debugger is attached to the same plugin/process/connection generation and the plugin validator accepts the exact request. Add Breakpoint defaults to persistent Software/Execute; data-row Add Watchpoint defaults to persistent Hardware/Write using the row value width. Rev35 extends the same workflow to Disassembler with separate Add Breakpoint/Add Watchpoint entries, and rev38 removes the unnecessary current/trigger-row restriction: breakpoint availability requires a valid executable instruction, while watchpoint availability requires a Paused matching debugger with current registers, one safe plugin-derived effective memory target for the single selected instruction, and final plugin validation. Invalid, ambiguous, stale, multi-selected, or unavailable actions remain disabled rather than attempted.
...
### Rev42 — Call Stack Group Creation and Application Icon — current candidate

- **[TK87ME — Implemented / current rev42 candidate] Explicit group creation row.** Each snapshot Group dropdown now opens with a dedicated new-group text input as the first popup row. Entering a non-empty name and pressing Enter registers/reuses the name through the existing case-insensitive comparer-session catalog and assigns it immediately to that snapshot. Existing groups remain normal choices below the input.
- **[TK87ME — Implemented / retained] Group membership semantics.** Snapshot rows still own their own Group metadata. Assigning the same name to two or more snapshots makes them members of the same input group for the existing grouped comparison engine. Group A/B remain non-editable selectors and cannot create names.
- **[TK87ME — Implemented / current rev42 candidate] Application icon.** The user-supplied TK artwork is stored as PNG source plus a generated multi-size ICO. The ICO is embedded as the executable icon and assigned consistently to WPF windows.
- **[TK87ME — Automated gate pending] Rev42 acceptance.** Require clean Windows build and 152/152, then repeat the focused comparer group UI/runtime gate that exposed rev41's discoverability issue. Verify the application icon in at least MainWindow and one modeless/dialog window before returning to the carried Export-dialog and Debugger Universal Export gates.

### Rev41 — Call Stack Comparer Snapshot Group Assignment — automated gate passed; group-creation UI superseded by rev42

- **[TK87ME — Implemented / retained by rev42] Saved-Addresses-style snapshot rows.** Call Stack Comparer snapshots use always-visible in-row editors for Label/Notes, a Group dropdown, centered read-only evidence fields, a per-row Danger Remove action, and an empty-list overlay consistent with the Saved Addresses table.
- **[TK87ME — Implemented / current rev42 candidate] Session-local group catalog.** Creating a new name in any snapshot Group dropdown registers it once for that comparer workspace; choosing an existing name assigns that snapshot to the same group. Names are reused case-insensitively, imported Group metadata is registered, and reassignment changes only the selected snapshot metadata. The catalog is not application-global and is recreated with a new comparer session.
- **[TK87ME — Implemented / current rev42 candidate] Restricted group comparison selectors.** Group A and Group B are non-editable dropdowns populated only from the comparer-session group catalog. New group names cannot be created from these selectors. Multiple snapshots carrying the same group name are passed together to the existing grouped comparison engine.
- **[TK87ME — Automated PASS / runtime UI superseded] Rev41 acceptance.** Rev41 passed 152/152. Its first runtime comparer check exposed that the editable Group ComboBox did not present a discoverable group-creation surface; rev42 replaces that interaction with an explicit first-row text input before group runtime verification continues.

### Rev40 — Universal Export Option Display Fix — automated gate passed, runtime/UI carried into rev42

- **[TK87ME — Implemented / 152/152 automated PASS] Shared export option presentation.** `DataExportDialog` renders Scope and Format choices through explicit `DisplayName` item templates, preventing record diagnostic representations from appearing in any consumer.
- **[TK87ME — Verification pending / carried into rev42] Cross-consumer UI regression.** Verify Scan Results, Saved Addresses, Disassembler, Debugger, and Call Stack Comparer result export all show clean scope/format labels.
- **[TK87ME — Resume point] Debugger Universal Export.** After the rev42 comparer UI/group gate and carried rev40 export presentation gate pass, continue the interrupted debugger export verification with Threads, Registers, Breakpoints/Watchpoints, Call Stack, and Events in JSON plus at least one tabular format.

### Rev39 — Call Stack Comparer Session Persistence and Explicit Capture — implemented / runtime verified through snapshot import safety

- **[TK87ME — Implemented / retained by current rev42 candidate] Disassembler debugger actions.** Exactly one selected instruction can expose separate **Add Breakpoint** and **Add Watchpoint** actions against an already attached same-target debugger. Multi-selection disables both and no implicit attach is performed. Add Breakpoint uses the existing Software/Execute request/validator path. Add Watchpoint requires a matching Paused debugger with current registers plus Plugin API `2.18.0` optional platform-owned resolution of one unambiguous memory operand, then reuses the existing neutral validator/add path for the derived address/width/access tuple. Rev38 permits any single safely resolvable instruction in the current Disassembler view rather than only the current stop/trigger row. Unsafe or ambiguous instructions are disabled rather than guessed.
- **[TK87ME — Implemented / candidate] Immutable Debugger Snapshot.** Core owns the complete neutral capture model and section status. A paused capture is published only after target/generation/stop-sequence revalidation and contains structured event/register/call-stack/logical-disassembly/breakpoint context plus bounded standard stack memory when available.
- **[TK87ME — Implemented / candidate] Lossless JSON persistence.** Snapshot schema version 1 preserves exact addresses/raw bytes and can be imported for offline analysis without connecting to or modifying a target.
- **[TK87ME — Implemented / candidate] Flat debugger Universal Export.** Threads, Registers, Breakpoints/Watchpoints, Call Stack, and Events use the existing table export pipeline independently from full snapshot JSON.
- **[TK87ME — Implemented / extended by current rev42 candidate] Call Stack Comparer.** A separate modeless workspace consumes the same `DebuggerSnapshot` model for live captures and imported files, supports pairwise and user-defined group comparison, normalizes module-relative code locations, and reports explainable stable differences without assigning unproven semantic names. Rev39 makes capture explicitly user-controlled and the workspace session-owned: opening/reopening the presentation window does not capture or discard evidence, **Capture Current** follows each valid Paused stop, and closing the Debugger removes live authority while an already-open comparer can continue offline.
- **[TK87ME — Deferred / optional] Deep Snapshot pointer-memory analysis.** Recursive/selected pointer-context expansion remains a later bounded extension and does not block the initial debugger-completion gate.
- **[TK87ME — Partially runtime verified / carried forward] Acceptance.** Rev39's 152-check gate and snapshot/comparer runtime path passed through JSON round-trip, offline ownership, invalid schema/version rejection, pairwise comparison, and same-session persistence. Remaining grouped-comparison, Universal Export, lifecycle/teardown, and final regression work is carried into `docs/testing/APP_0.1.7_REV42_VERIFICATION.md`.
...
- **[TK87ME — Implemented / current rev42 candidate] Universal Export for threads, registers, call frames, breakpoints/watchpoints, and debug events through the existing shared tabular export pipeline. Rev40 corrected the shared Scope/Format option presentation and passed 152/152; its cross-consumer UI check is carried into rev42 before debugger export verification resumes. Complete debugger context remains a separate hierarchical Debugger Snapshot JSON format.**
...
31. **rev31 — Safe PS5 Watchpoint Detach Cleanup — verified:** PS5 `0.1.0.rev38`; explicitly disables every active or staged client-owned hardware-watchpoint slot before detach/disposal; the full **142/142** automated gate and focused live teardown regressions are accepted.
32. **rev32 — Watchpoint Trigger Resolution and Event Semantics — implemented / verification carried forward:** separate authoritative stop/current IP from trigger instruction, add explicit trigger-resolution state and watched-address/access/size event context, move resolved `Watchpoint hit` markers to the actual trigger instruction, keep unresolved stops honest, add deterministic Mock backend-exact coverage, and preserve all rev31 teardown safety. Registry target: **143/143**.

**Current debugger-finalization state:** rev33 implemented the immutable Debugger Snapshot Core model, shared paused-session capture service, lossless snapshot JSON serializer/importer with transactional publication, Universal Export for Threads/Registers/Breakpoints-Watchpoints/Call Stack/Events, the modeless Call Stack Comparer, and pairwise/user-defined grouped comparisons. Rev38 passed 152/152 and the focused live PS5 Disassembler action gates. Rev39 corrected comparer lifecycle/explicit capture and runtime verification passed through snapshot JSON round-trip, offline ownership, and invalid schema/version rejection. Rev40 corrected shared Export-dialog option presentation and passed 152/152. Rev41/rev42 established the Saved-Addresses-style snapshot rows, discoverable session-local group creation, and application icon; rev42 then completed the carried cross-consumer Export-dialog, debugger Universal Export, group-comparison, offline/modeless lifecycle, and focused regression runtime gates. That runtime pass exposed a final set of comparer action-state/stale-result issues plus shared export-column UX gaps. Rev43 addresses those findings with exact-selection/group gating, confirmed removal, explicit No group, comparison-input invalidation, Export Results validity gating, Select None/live Continue validation, and Primary styling for Main Window Disassembler/Debugger. Optional bounded Deep Snapshot pointer-memory analysis remains deferred. Rev44 corrects the rev43 automated-test compile failure caused by two fixture-path locals being declared in the wrong verification method; the rev43 runtime implementation is otherwise unchanged. Rev45 follows the focused rev44 runtime findings by gating Debugger Export to attached state, repairing No group membership clearing, and adding Description-aware Saved Address single-remove confirmation. Rev46 corrects the rev45 verification-project `CS0103` caused by an undeclared fixture-source identifier without changing runtime behavior. Rev46 then passed the Windows automated gate at **152/152** and the bounded five-check runtime pass confirmed Debugger Export attach gating, Saved Addresses conditional single-remove confirmation, pairwise stale-result invalidation with Export Results gating, and Universal Export Select None/live Continue state. The only remaining runtime defect was visual: `No group` cleared and persisted the model correctly but left the old group displayed until the comparer reopened. Rev47 attempted a focused `SelectedItem` binding-target refresh, but runtime verification showed that the active ComboBox selection box still retained the old group because the ungrouped empty value is intentionally absent from `AvailableGroups`. Rev48 replaces that ineffective workaround with an explicit live selection reset (`SelectedIndex = -1`) after the already-correct model clear, while preserving the two-way group binding and session group catalog. The remaining work before `0.1.7` can be declared complete is the focused rev48 runtime confirmation, permanent teardown safety confirmation where required by the final closeout plan, and the final full `0.1.7` regression/closure review.
