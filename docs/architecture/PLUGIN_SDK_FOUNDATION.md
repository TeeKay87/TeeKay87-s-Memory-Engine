...
Application `0.1.7.rev1` deliberately left both built-in plugin implementations unchanged and established the API `2.12.0` debugger surface. Application `0.1.7.rev2` made Mock `1.0.0.rev8` the first consumer through `IDebuggerProvider`/`IDebuggerSession`. The real PS5 consumer arrived in rev3 as plugin `0.1.0.rev25`, and that transport/lifecycle implementation was fully hardware-verified in host `0.1.7.rev5`. Application `0.1.7.rev6` keeps Plugin API `2.12.0` unchanged and makes the already-defined thread contracts production consumers for the first time: Mock `1.0.0.rev9` and PS5 `0.1.0.rev26` advertise `Debugger + ThreadEnumeration + ThreadControl` and expose the corresponding attached-session services. Application `0.1.7.rev7` advances the API to `2.13.0` and activates `RegisterAccess` in Mock `1.0.0.rev10` and PS5 `0.1.0.rev27`. The host consumes only neutral register bytes, encoding, writability, group, and semantic roles; PS5 keeps its amd64 register layout private and exposes read-only GP snapshots in this revision. Breakpoints, Watchpoints, CallStack, and StepExecution remain unadvertised until their ordered milestones are implemented and verified.
