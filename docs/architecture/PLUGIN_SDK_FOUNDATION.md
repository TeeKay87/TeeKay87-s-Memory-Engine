...
## Host `0.1.7.rev32` Compatibility Note

Plugin API `2.17.0` extends neutral debugger events with watchpoint trigger semantics. `DebuggerTriggerResolution` defines `Unresolved`, `BackendExact`, and `DisassemblyDerived`. `DebuggerEvent` can now carry `TriggerInstructionAddress` independently from `InstructionPointer`, and watchpoint events expose watched address, access mode, and size through neutral properties derived from the triggered watchpoint request.

The existing `DebuggerEvent` constructors remain present. Older compatible 2.x plugins can continue producing events without trigger metadata; those events enter the host as `Unresolved`. A backend that can identify the exact trigger may use the new constructor and `BackendExact`. Host/Core may enrich unresolved events from logical disassembly without changing the original stop/current IP.


## Host `0.1.7.rev33` Compatibility Note

Host `0.1.7.rev33` kept Plugin API `2.17.0` unchanged. Debugger snapshots, JSON persistence, table exports, and comparison remain host/Core/WPF composition over neutral services. Host `0.1.7.rev35` advances the API to `2.18.0` only for optional Disassembler watchpoint-target derivation. No plugin is required to construct snapshots or implement a comparison API, and a compatible plugin that does not expose `IDisassemblyWatchpointResolver` simply leaves automatic Disassembler **Add Watchpoint** unavailable.

## Host `0.1.7.rev36` Compatibility Note

Host `0.1.7.rev36` keeps Plugin API `2.18.0` unchanged. The revision is a PS5 implementation compile correction inside `IDisassemblyWatchpointResolver`: unsupported Iced address-register forms now explicitly assign the `out` value before returning `false`. No public contract, neutral model, capability, or host fallback behavior changes.


### Disassembly-derived watchpoint target resolution (`2.18.0`)

`IDisassemblyWatchpointResolver` is an optional target-session service used only when the host needs to turn a selected disassembled memory-access instruction into a concrete hardware data-watchpoint request.

The resolver receives:

- one neutral `DisassembledInstruction` already produced by the plugin's disassembler;
- the current neutral `DebuggerRegister` snapshot from the matching paused debugger context.

It returns either one `DisassemblyWatchpointTarget` containing `Address`, `Size`, and non-Execute `DebuggerBreakpointAccess`, or `null` when the target cannot be established safely. Platform/architecture-specific decoding, operand semantics, segment rules, and effective-address calculation remain plugin-owned. The host never parses x86 operand text. The returned tuple is still passed through the existing `IDebuggerBreakpointValidationService` before the watchpoint is added, so the resolver does not bypass backend width/alignment/range/slot restrictions.
