...
Application `0.1.6.rev14` is the fully tested and hardware-verified final host implementation for the `0.1.6` Disassembler block. The main workspace exposes **Disassembler...** only when the selected plugin advertises `TargetCapabilities.Disassembly` and the current Active Target can provide the neutral memory/disassembly services. The workspace captures plugin id, process identity, and connection generation and revalidates them before every read so reconnect/target replacement cannot silently redirect an existing window.
