...
The current application build is **0.1.7.rev7 - Registers and Stop Context**. The `0.1.3` scanner block remains user-verified through rev32, the `0.1.4` universal-export block is fully verified through rev5, the `0.1.5` Memory Viewer block is fully verified through rev8, and the complete `0.1.6.rev14` Disassembler block is fully tested and hardware-verified. `0.1.7.rev1 - Debugger Contracts and Core Foundation` is verified with **84/84** Windows checks, and `0.1.7.rev2 - Debugger Workspace and Mock Backend` is fully verified with **89/89** Windows checks plus the complete focused Mock Debugger runtime/UI acceptance. The rev3 PS5 debugger backend was carried forward through the superseding UI revisions, and `0.1.7.rev5 - Responsive Target Header Input Sizing` is fully verified with **97/97** Windows checks plus the complete responsive-header/live-PS5 debugger acceptance. `0.1.7.rev6 - Threads and Thread Control` then passed **101/101** Windows checks and all focused Mock/live-PS5 gates except individual PS5 thread Suspend/Resume, which is implemented in Memory Engine but blocked by the current ps5debug-NG backend returning `CMD_ERROR` for the documented suspend request. Thread enumeration, whole-target Pause/Continue, transport isolation, cleanup, and the final PS5 regression smoke all passed. Rev7 builds on that verified baseline by activating the neutral register service, adding paused-thread register snapshots and safe register editing in the deterministic Mock backend, adding real read-only PS5 general-register snapshots through ps5debug-NG `GETREGS`, and integrating the semantic instruction-pointer role with the existing Disassembler launcher. The public Plugin API advances to `2.13.0` because register values now carry explicit neutral byte/numeric encoding metadata for portable presentation and editing.
...
| --- | --- | --- | --- |
| In-Memory Test Target | `1.0.0.rev10` | `2.13.0` | Deterministic development/regression target using Core-owned Scan Types, plugin-owned Value Types, synthetic custom/unknown 64-bit disassembly, debugger attach/pause/continue/events, deterministic thread control, and writable paused-thread register snapshots |
| PlayStation 5 | `0.1.0.rev27` | `2.13.0` | Connection with remembered successful host/port, preferred `eboot.bin` process selection, memory access, Core-owned Scan Types, plugin-owned Value Types/options, semantic Core-to-native TurboScan mappings, deferred server-resident result materialization with automatic Core fallback, process suspend/resume, a dedicated concurrent Frozen-write channel, plugin-owned x86-64 disassembly, dedicated ps5debug-NG debugger transport, real thread enumeration/control requests, and read-only general-register snapshots while paused |
...
- reusable theme-aware confirmation dialogs with configurable title, message, affirmative/cancel button text, semantic Information/Warning/Danger tone, and safe default-button behavior; the Saved Addresses **Remove All** action is the first production caller;
...
- shared Core `DisassemblyReader`, `DisassemblySnapshot`, and `DisassemblySessionIdentity` primitives: reads are clamped to one readable non-guarded region through `IMemoryReader`, provider output is range/order/raw-byte validated, and future workspace instances can bind safely to plugin/process/connection-generation identity;
- Mock plugin `1.0.0.rev10` continues to provide the deterministic synthetic Disassembler/debugger fixtures and advertises `Debugger`, `ThreadEnumeration`, `ThreadControl`, and `RegisterAccess`; its attached debugger session exposes the existing neutral pause/resume events, deterministic Main/Worker/Render threads, per-thread Suspend/Resume while Running, and deterministic writable 64-bit register snapshots while Paused; breakpoint/watchpoint/call-stack/step capabilities remain unadvertised;
- PS5 plugin `0.1.0.rev27` targets Plugin API `2.13.0`, retains the verified Iced `1.21.0` x86-64 Disassembler provider and debugger transport, and advertises `Debugger`, `ThreadEnumeration`, `ThreadControl`, and `RegisterAccess`. While the target is Paused, the attached session requests the selected thread's 176-byte ps5debug-NG general-register block and maps it to neutral register rows with semantic InstructionPointer/StackPointer/FramePointer roles. PS5 register rows are intentionally read-only in rev7 because the upstream SETREGS path is not yet verified. Protocol ids, LWP terminology, FreeBSD register layout, and register offsets remain entirely inside the PS5 plugin;
- capability-driven modeless Disassembler workspace introduced in host `0.1.6.rev3` and extended through the completed `0.1.6.rev14` block: the main target strip exposes **Disassembler...** only for plugins advertising `Disassembly`; the workspace binds to the Active Target/process/connection generation, routes reads through the existing foreground target reservation, accepts hexadecimal Go To/Refresh reads, supports successful-address Back/Forward history, shows Region / Module, visible range, Protection, Architecture, and the same virtualized Address/Bytes/Instruction table; the default view contains up to 512 bytes before plus 512 bytes from the requested origin, decodes that complete range continuously, marks the decoded instruction containing the requested address with the existing theme-aware green origin surface, and applies theme-aware syntax coloring inside the Instruction cell without changing column structure or row dimensions;
- the rev12-rev14 Disassembler workflow provides extended row selection with right-click selection preservation, selection-consistent granular copy commands, Displayed/Selected universal export through the existing JSON/CSV/TSV/Markdown pipeline, Previous/Region Start/Region End/Next readable-region navigation through the shared Core navigator, real module-relative origin text when module metadata exists, and direct Follow Target navigation through the same successful-address history;
...
- Plugin API `2.12.0` introduced the neutral debugger provider/session boundary plus optional attached-session contracts for thread enumeration, separate thread control, registers, breakpoints/watchpoints, call stacks, and stepping; Plugin API `2.13.0` adds neutral `DebuggerRegisterValueEncoding` metadata so register byte snapshots can be presented and edited without architecture-specific parsing in WPF, while retaining the previous seven-parameter `DebuggerRegister` constructor for compatible compiled plugins;
...
- the verified rev1 foundation adds `TargetCapabilities.ThreadControl` so plugins can distinguish thread enumeration from individual-thread suspend/resume; rev6 activated `ThreadEnumeration`/`ThreadControl`, and rev7 activates `RegisterAccess` in Mock and PS5 while Breakpoints, Watchpoints, CallStack, and StepExecution remain unadvertised until their services are implemented;
- capability-driven modeless Debugger workspace introduced in `0.1.7.rev2` and extended through rev7: **Debugger...** appears only for plugins advertising `Debugger`; each window is bound to one Active Target and connection generation, uses the shared Core coordinator for explicit Attach/Pause/Continue/Detach, displays a bounded virtualized neutral event history, rejects stale target/event contexts, conditionally adds a host-neutral Threads pane, and now shows a capability-driven Registers pane only while register access is available; register snapshots are cleared as soon as the target resumes or the bound session becomes stale;
...
- compact responsive target header: the upper target/connection surface contains exactly two permanent control rows. Row 1 is Platform -> plugin-declared connection inputs -> Connect -> Disconnect -> Target Process -> Refresh -> Set Active Target. Row 2 is Reload Plugins -> Disassembler -> Debugger, with future top-level tool/action buttons appended to that same second row. Platform, Target Process, and every ordinary plugin-declared TextBox/ComboBox in row 1 share one host-computed responsive width capped by `UiMetrics.TopTargetInputMaxWidth = 180`; there is no minimum width, so the controls shrink together when the row is constrained after fixed actions and existing left/right/inter-control margins are reserved. The redundant `Active <process>` and process-count/enumeration hint text remain removed, and connection state remains at the far left of the permanent bottom status bar in a theme-aware red/green indicator;
...
Variable-length architectures still cannot prove that the first byte of an arbitrary raw context window is the program's canonical instruction boundary, so the workspace states that limitation explicitly. The requested origin itself is resolved against the decoded instruction span, preserving the live-verified mid-instruction behavior from rev5.

### Debugger workspace

Application `0.1.7.rev2` introduced the first modeless Debugger workspace on top of the verified rev1 Plugin SDK/Core foundation, rev3 connected the same neutral workspace to the real PS5 backend, rev5 completed live hardware verification of that transport, rev6 added the first advanced debugger thread service surface, and rev7 adds paused-thread registers/stop context without changing the underlying host/session architecture. The **Debugger...** button appears only when the selected plugin advertises `TargetCapabilities.Debugger`; opening a window requires a connected current Active Target and captures that process plus the current connection generation. Opening the window never attaches automatically.

The workspace provides explicit **Attach**, **Pause**, **Continue**, **Detach**, and **Clear Events** actions. Lifecycle/control calls are routed through `DebuggerSessionCoordinator`, not directly to a platform backend. The event table is read-only and virtualized and shows sequence, time, event kind, execution state, stop reason, thread id, instruction pointer, and neutral message. The displayed history is bounded to the newest 2,000 events.
...
Mock plugin `1.0.0.rev10` exposes the deterministic Main/Worker/Render thread fixture plus writable paused-thread register snapshots. PS5 plugin `0.1.0.rev27` retains the hardware-verified dedicated command/TCP 755 event transport, thread enumeration, and whole-target controls, and adds read-only paused-thread general-register snapshots through ps5debug-NG GETREGS. Individual PS5 thread Suspend remains client-implemented but backend-blocked on the tested payload, which returns `CMD_ERROR`; the detailed reproduction is under `docs/bug-reports/`. Current ps5debug-NG does not return a per-thread run-state field with list/info records, so ordinary Running/Stopped projection remains plugin-private.

Every Debugger window is permanently bound to the plugin/process/connection generation captured when it was opened. The host owns all coordinators created for the current target session and disposes them before Disconnect, target-session replacement, Active Target changes, or application/plugin shutdown. An old window therefore becomes stale instead of silently following a newly connected process. Register access, breakpoints/watchpoints, call stacks, and stepping remain later debugger revisions.
...
1. **Application version** — TeeKay87's Memory Engine, currently `0.1.7.rev5`;
2. **Plugin version** — each plugin has its own semantic version and revision;
3. **Plugin API version** — compatibility version for the public host/plugin contract, currently `2.13.0`.
...
These fixtures support scanner, Saved Addresses, freeze, export, Memory Viewer, disassembly, debugger, and regression testing. The established health/ammo/money addresses remain unchanged. Mock plugin `1.0.0.rev8` exposes the existing deterministic synthetic instruction fixture at `0x10000400` and now also uses that address as the deterministic instruction pointer reported by its debugger pause/resume event flow. Attach starts Running; Pause reports thread `0x1` at `0x10000400`; Continue returns to Running. The plugin exposes no advanced debugger services yet.
...
The WPF host renders these settings from Plugin SDK metadata; it does not contain PS5-specific IP or port logic. PS5 plugin `0.1.0.rev25` also consumes its Core-managed `IPluginSettings` scope. After a connection succeeds, the plugin stores the normalized host and port as `connection.host` and `connection.port`. On later discovery those remembered values become the connection-field defaults. Failed connection attempts and unfinished textbox edits do not replace the last successful values.
...
The current revision contains **97 checks**. A successful run ends with `All 97 checks passed.`. All 96 registrations from the rev4 candidate remain present, and rev5 adds one focused source contract for responsive target-input sizing while updating the existing two-row header contract to require the shared 180-unit maximum and responsive binding. No earlier scanner, storage, export, Memory Viewer, Disassembler, plugin, target-session, protocol, debugger-foundation, PS5-debugger, target-header, or shared-button regression is removed or weakened.

Project-wide verification documents are kept under [`docs/testing/`](docs/testing/). The current revision checklist is [`docs/testing/APP_0.1.7_REV5_VERIFICATION.md`](docs/testing/APP_0.1.7_REV5_VERIFICATION.md). The current static source review is [`docs/testing/APP_0.1.7_REV5_SOURCE_REVIEW.md`](docs/testing/APP_0.1.7_REV5_SOURCE_REVIEW.md). The completed rev2 debugger-workspace verification remains recorded in [`docs/testing/APP_0.1.7_REV2_VERIFICATION.md`](docs/testing/APP_0.1.7_REV2_VERIFICATION.md), and the completed Disassembler feature-block acceptance is recorded in [`docs/testing/APP_0.1.6_FINAL_VERIFICATION.md`](docs/testing/APP_0.1.6_FINAL_VERIFICATION.md). Plugin-specific runtime/protocol verification belongs under each plugin's dedicated `docs/plugins/<Plugin>/` directory.
