...
The current application build is **0.1.7.rev31 - Safe PS5 Watchpoint Detach Cleanup**. Rev29's logical Disassembler presentation remains verified and unchanged, while rev30's Breakpoint/Watchpoint **Type** versus Software/Hardware **Mechanism** split and validated Scan Results/Saved Addresses **Add Breakpoint** / **Add Watchpoint** shortcuts remain intact. Rev31 corrects PS5 debugger teardown after live testing showed that an enabled hardware watchpoint could survive client exit and later terminate the game when the watched access occurred. PS5 plugin `0.1.0.rev38` now explicitly disables every client-owned backend-active or staged hardware-watchpoint slot before backend detach/disposal, matching the defensive software-breakpoint cleanup already established in rev36 instead of relying solely on ps5debug-NG's teardown rediscovery. Plugin API remains `2.16.0`; Mock remains `1.0.0.rev16`. The automated registry target is **142 checks**. Individual PS5 thread Suspend/Resume remains implemented on the client but externally blocked by the tested ps5debug-NG backend returning `CMD_ERROR`.
...
- Mock plugin `1.0.0.rev16` provides the deterministic synthetic Disassembler/debugger fixtures and advertises `Debugger`, `ThreadEnumeration`, `ThreadControl`, `RegisterAccess`, `Breakpoints`, `Watchpoints`, `CallStack`, and `StepExecution`; its attached debugger session exposes neutral pause/resume events, deterministic Main/Worker/Render threads, per-thread Suspend/Resume while Running, writable 64-bit General/Control register rows while Paused, read-only 80-bit floating-point, 128/256-bit SIMD and debug-register fixtures, persistent/temporary software execute breakpoints, deterministic Read/Write/Read-Write hardware watchpoints with a four-slot limit, deterministic three-frame call stacks, and native Step Into while Paused;
- PS5 plugin `0.1.0.rev38` targets Plugin API `2.16.0`, retains the verified Iced `1.21.0` x86-64 Disassembler provider and dedicated debugger transport, and advertises `Debugger`, `ThreadEnumeration`, `ThreadControl`, `RegisterAccess`, `Breakpoints`, `Watchpoints`, `CallStack`, and `StepExecution`. The plugin keeps the matching software-breakpoint interrupt's general/FPU packet snapshot as the authoritative logical stop context and consumes ps5debug-NG's already-completed transparent breakpoint step when Step Into is requested from that stop. Rev36 performs defensive software-breakpoint restoration before detach/disposal, and rev38 extends the same client-owned teardown rule to hardware watchpoints: every backend-active watchpoint plus every staged temporary-watchpoint cleanup is explicitly disabled through the existing watchpoint command before backend detach. The event channel remains open while teardown commands run so backend-generated interrupts can be drained and ignored while detaching. Ordinary paused register reads still use the debugger-owner GETREGS path, safe optional FPU/FS/GS reads remain capability-gated, paused GETDBREGS remains suppressed, and all PS5 register rows remain read-only;
...
The WPF host renders these settings from Plugin SDK metadata; it does not contain PS5-specific IP or port logic. PS5 plugin `0.1.0.rev38` also consumes its Core-managed `IPluginSettings` scope. After a connection succeeds, the plugin stores the normalized host and port as `connection.host` and `connection.port`. On later discovery those remembered values become the connection-field defaults. Failed connection attempts and unfinished textbox edits do not replace the last successful values.
